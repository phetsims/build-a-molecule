...
define( require => {
  ...
  class SliceNode extends DOM {
    ...
    constructor( kit, viewSwipeBounds, view ) {
      ...
      this.sliceInputListener = new SimpleDragHandler( {
        ...
        start: ( event, trail ) => {
          ...
          this.kit.molecules.forEach( molecule => {
        ...
        drag: ( event, trail ) => {
          ...
          this.bondData.forEach( dat => {
            if ( dat.cut ) {
              cutCount++;
              this.kit.breakBond( dat.bond.a, dat.bond.b, true ); // skip the destination separation
            }
          } );
          if ( cutCount ) {
            this.kit.separateMoleculeDestinations();
          }
          this.traveledDistance = 0;
          this.bondData = [];
    ...
    /**
     * Swap the kit associated with the slice node
     *
     * @param activeKit
     * @public
     */
